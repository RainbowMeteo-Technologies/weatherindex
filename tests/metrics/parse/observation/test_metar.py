...
class TestMetarParser:
    ...
    @pytest.mark.parametrize("sample_xml, report_timestamp, expected_rows", [
        ...
        # freezing rain (descriptor + rain atom in one weather group)
        (
            _create_metar_file(raw_text="TJBQ 251150Z 00000KT 10SM FZRA SCT015 25/21 A2990",
                               station_id="TJBQ",
                               observation_time="2024-03-25T11:50:00Z",
                               lat=18.494,
                               lon=-67.128,
                               ...
                               wind_dir_degrees=5.4,
                               wind_speed_kt=23.0,
                               sky_condition=['<sky_condition sky_cover="SCT" cloud_base_ft_agl="1500" />']),
            1725667200,

            [
                ["TJBQ", -67.128, 18.494, 1711367400, 10.0, PrecipitationType.RAIN.value, 33, 78, 40, 57,
                 [(SkyCover.SCT.value, 1500)]]
            ]
        ),
        # hail
        (
