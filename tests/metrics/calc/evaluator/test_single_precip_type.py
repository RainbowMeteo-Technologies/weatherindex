...
class TestRainOnlyEvaluator:
    def get_evaluator(self) -> RainOnlyEvaluator:
        return RainOnlyEvaluator()

    @pytest.mark.parametrize("forecast_offsets, observations, forecast, expected_metrics", [
        (
            [0, 600, 1200],
            create_observations([
                ("sensor_1", 10.0, PrecipitationType.RAIN.value, timestamp(0)),
            ...
            create_metrics_result(data=(
                ...
                (600, "sensor_1", *create_metrics(tp=1)),
                (1200, "sensor_1", *create_metrics(tp=1)),
                (0, "sensor_2", *create_metrics(fp=1)),
                (600, "sensor_2", *create_metrics(fp=1)),
                (1200, "sensor_2", *create_metrics(fp=1))
            ), columns=("forecast_time", "sensor_id", "tp", "tn", "fp", "fn"))
        ),
        (
            [0],
            create_observations([
                ("X", 10.0, PrecipitationType.RAIN.value, timestamp(0)),
    ...
    @pytest.mark.parametrize("forecast_offsets, observations, forecast, expected_metrics", [
        (
            ...
            create_observations([
                ("sensor_1", 10.0, PrecipitationType.RAIN.value, timestamp(0)),
                ("sensor_2", 0.0, PrecipitationType.RAIN.value, timestamp(0)),
                ("sensor_3", 10.0, PrecipitationType.RAIN.value, timestamp(0))
            ]),
            create_forecast([
...
class TestSnowOnlyEvaluator:
    def get_evaluator(self) -> SnowOnlyEvaluator:
        return SnowOnlyEvaluator()
    ...
    @pytest.mark.parametrize("forecast_offsets, observations, forecast, expected_metrics", [
        (
            ...
            create_observations([
                ...
                ("Y", 10.0, PrecipitationType.SNOW.value, timestamp(0)),
            ...
            create_metrics_result(data=[
                (0, "X", *create_metrics(tp=1)),
                (0, "Y", *create_metrics(fn=1)),
                (0, "Z", *create_metrics(fp=1))
            ], columns=("forecast_time", "sensor_id", "tp", "tn", "fp", "fn"))
        ),
    ])
    def test_calculate_detailed(self,
